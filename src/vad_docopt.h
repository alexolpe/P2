...
"   -1 FLOAT, --alpha1=FLOAT    Increment per determinar el llindar de desició [default: 10]\n"
"   -2 FLOAT, --alpha2=FLOAT    Increment per determinar el segon llindar de desicio\n"
"   -r FLOAT, --refsil=FLOAT    Nombre de trames per analitzar el silenci\n"
"   -f FLOAT, --framer=FLOAT    Framerate\n"
...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_options; i++) {
        ...
        } else if (!strcmp(option->olong, "--framer")) {
            if (option->argument)
                args->framer = option->argument;
        ...
        } else if (!strcmp(option->olong, "--refsil")) {
            if (option->argument)
                args->refsil = option->argument;
        }
    }
    /* commands */
